...
use dioxus::prelude::{use_context, use_context_provider, use_resource};
...
impl AppState {
    ...
    fn build_network(mut self) {
        ...
        // Build `state.flake` signal dependent signals change
        {
            ...
            use_resource(move || {
                let value = flake_url.clone();
                async move {
                    if let Some(flake_url) = value {
        ...
        // Build `state.health_checks`
        {
            ...
            use_resource(move || {
                let value = nix_info.clone();
                async move {
                    if let Some(nix_info) = value.current_value().map(|x| {
                        x.as_ref()
                            .map_err(|e| Into::<SystemError>::into(e.to_string()))
                            .map(|v| v.clone())
                    }) {
