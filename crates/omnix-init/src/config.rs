...
#[serde(untagged)]
pub enum Action {
    /// A text prompt is for replacing text across all files.
    Replace {
        /// The text to replace.
        placeholder: String,
    ...
    /// A files prompts for including or excluding the given files.
    Retain {
...
pub fn set_defaults(prompts: &mut [Param], defaults: &HashMap<String, Value>) {
    for prompt in prompts.iter_mut() {
        if let Some(default) = defaults.get(&prompt.name) {
            match &mut prompt.action {
                Action::Replace { default: d, .. } => {
                    *d = default.as_str().map(|s| s.to_string());
                }
                Action::Retain { default: d, .. } => {
                    *d = default.as_bool();
                }
            }
        }
...
pub fn prompt_value(param: &Param) -> anyhow::Result<()> {
    match &param.action {
        Action::Replace {
            placeholder,
            default,
        } => {
            let mut p = inquire::Text::new(&param.name)
