...
use command::{BuildCommand, Command};
...
pub async fn nixci(
    ...
    match command {
        command::Command::Build(cmd) => {
            let cfg = command::Command::get_config(nixcmd, &cmd.flake_ref).await?;
        ...
        command::Command::DumpGithubActionsMatrix(cmd) => {
            let cfg = command::Command::get_config(nixcmd, &cmd.flake_ref).await?;
